...
Machine::Translate(int virtAddr, int *physAddr, int size, bool writing)
{
    ...
    { // => page table => vpn is index into table
        ...
        else if (!pageTable[vpn].valid)
        {
            ...
            pageNum = 0;
            while (kernel->machine->usedPhyPage[pageNum] != FALSE && pageNum < NumPhysPages)
            {
                pageNum++;
            }
            //add the page into the main memory if the main memory isn't full

            if (pageNum < NumPhysPages)
            {
                char *buf; //save page temporary
                buf = new char[PageSize];
                kernel->machine->usedPhyPage[pageNum] = TRUE;
                kernel->machine->PhyPageName[pageNum] = pageTable[vpn].ID;

                kernel->machine->main_tab[pageNum] = &pageTable[vpn];
                pageTable[vpn].physicalPage = pageNum;
                pageTable[vpn].valid = TRUE;
                if (!isFifo)
                    pageTable[vpn].lastUsedTime=(kernel->stats->totalTicks); //LRU

                kernel->virDisk->ReadSector(pageTable[vpn].virtualPage, buf);
                bcopy(buf, &mainMemory[pageNum * PageSize], PageSize);
            }
            else
            {
                char *buf_1;
                ...
                if (!isFifo)
                {
                    int last = pageTable[0].lastUsedTime;
                    victim = 0;
                    for (int j = 0; j < 32; j++)
                    {
                        if (last > pageTable[j].lastUsedTime)
                        {
                            last = pageTable[j].lastUsedTime;
                            victim = j;
                        }
                    }
                    pageTable[victim].lastUsedTime=(kernel->stats->totalTicks);
                }
                printf("Number:%d swap out\n", victim);

                //get the page victim and save in the disk
                bcopy(&mainMemory[victim * PageSize], buf_1, PageSize);
                kernel->virDisk->ReadSector(pageTable[vpn].virtualPage, buf_2);
                bcopy(buf_2, &mainMemory[victim * PageSize], PageSize);
                kernel->virDisk->WriteSector(pageTable[vpn].virtualPage, buf_1);

                main_tab[victim]->virtualPage = pageTable[vpn].virtualPage;
                main_tab[victim]->valid = FALSE;

                //save page into the main memory
